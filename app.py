...
def recursive_crawl(start_url, max_pages=5):
    visited = set()
    # ⚡ Bolt: Use collections.deque for O(1) pops and a set for O(1) queue lookups
    ...
    while queue and count < max_pages:
        ...
        # ⚡ Bolt: O(1) popleft from deque replaces O(N) list pop(0)
        url = queue.popleft()
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # ⚡ Bolt: Check O(1) sets before slow urlparse to save execution time
