...
def recursive_crawl(start_url, max_pages=5):
    visited = set()
    # ⚡ BOLT OPTIMIZATION: Replaced O(N) list operations with O(1) deque for pops and set for containment checks
    queue = deque([start_url])
    queued_urls = {start_url} # Tracks URLs already added to the queue to prevent duplicate processing
    combined_text = ""
    site_structure = {} 
    ...
    while queue and count < max_pages:
        ...
        url = queue.popleft() # ⚡ O(1) pop instead of O(N) pop(0)
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # ⚡ O(1) containment check instead of O(N) `not in list` search
