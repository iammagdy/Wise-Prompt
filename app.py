...
def recursive_crawl(start_url, max_pages=5):
    visited = set()
    # OPTIMIZATION: Use deque for O(1) pops instead of O(N) list.pop(0)
    queue = deque([start_url])
    # OPTIMIZATION: Auxiliary set for O(1) queue membership checks
    ...
    while queue and count < max_pages:
        ...
        # OPTIMIZATION: O(1) popleft from deque
        url = queue.popleft()
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # OPTIMIZATION: Fast O(1) set membership checks BEFORE slow O(N) string operations like urlparse
