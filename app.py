...
def recursive_crawl(start_url, max_pages=5):
    # OPTIMIZATION: Use deque for O(1) pops from the left
    visited = set()
    queue = deque([start_url])
    # OPTIMIZATION: List accumulation is faster than string concatenation
    combined_text_list = []
    site_structure = {}
    ...
    with requests.Session() as session:
        while queue and count < max_pages:
            ...
            url = queue.popleft()  # OPTIMIZATION: popleft is O(1)
            if url in visited:
                continue
            ...
            try:
                ...
                def extract_assets_internal(soup, url):
                    ...
                    for link in soup.find_all('link', href=True):
                        href = link['href']
                        if 'fonts.googleapis.com' in href or href.endswith(
                                '.woff'):
                ...
                # OPTIMIZATION: Append to list instead of string concatenation
                ...
                for link in soup.find_all('a', href=True):
                    ...
                    if urlparse(
                            full_url).netloc == base_domain and full_url not in visited and full_url not in queue:
    ...
    # OPTIMIZATION: Join list at the end
...
with tab1:
    ...
    if mode == "⚡ Vibe Coder (Bolt/Antigravity)":
        ...
        complexity = st.select_slider(
            "DEPTH", [
                "Standard", "Detailed", "God-Mode"], value="God-Mode", key="tab1_slider")
...
with tab2:
    ...
    with c1:
        url = st.text_input(
            ...
            key="tab2_url")
    with c2:
        crawl_scope = st.selectbox("SCOPE",
                                   ["Home Page Only",
                                    "Quick Scan (5 Pages)",
                                    "Deep Scan (20 Pages)",
    ...
    if st.button("🕷️ INITIATE SCAN", type="primary", key="tab2_btn"):
        if not url:
            ...
            st.session_state.messages = [
                {
                    "role": "assistant",
                    "content": f"**SCAN COMPLETE.** Analyzed {
                        stats['pages']} pages. Found {
                        stats['buttons']} buttons. Ready for queries."}]

            add_to_history(
                api_key, "Web Scanner", url, f"Scanned {
                    stats['pages']} pages. Found {
                    len(
                        assets['fonts'])} fonts.")
            st.rerun()
        ...
        if user_input := st.chat_input("QUERY DATABASE...", key="tab2_chat"):
            ...
            with st.chat_message("assistant"):
                with st.spinner("COMPUTING..."):
                    chat_prompt = f"Expert Architect. KB: {
                        st.session_state.knowledge_base[
                            :30000]}. Q: {user_input}"
