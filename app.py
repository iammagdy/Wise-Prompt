...
def recursive_crawl(start_url, max_pages=5):
    visited = set()
    # OPTIMIZATION: Use deque for O(1) pops from the left (BFS)
...
with tab2:
    ...
    with c1:
        url = st.text_input("TARGET URL", placeholder="https://example.com", key="tab2_url")
    with c2:
...
with tab3:
    ...
    c1, c2 = st.columns(2)
    with c1:
        stack = st.selectbox("TECH STACK", ["Next.js + Tailwind", "React + Three.js", "HTML/CSS"], key="tab3_stack")
    with c2:
        vibe = st.text_input("VIBE", placeholder="Cyberpunk, Clean", key="tab3_vibe")
